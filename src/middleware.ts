...
const RATE_LIMITS = {
  ...
  auth: { requests: 20, window: 60 * 1000 }, // 20 requests per minute (increased for testing)
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/') && ['POST', 'PUT', 'DELETE', 'PATCH'].includes(request.method)) {
    ...
    const skipCSRFPaths = ['/api/auth/', '/api/login', '/api/signup', '/api/webhook', '/api/csrf-token', '/api/geocode', '/api/admin/', '/api/debug/', '/api/setup-test-accounts'];

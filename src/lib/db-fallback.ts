    ...
    if (dbAvailable) {
      ...
      try {
        ...
        if (params.latitude && params.longitude) {
          ...
          const minRate = params.minRate != null ? Number(params.minRate) : null;
          const maxRate = params.maxRate != null ? Number(params.maxRate) : null;
          const minRating = params.minRating != null ? Number(params.minRating) : null;

          // Use $queryRaw tagged template for safe parameterized queries
          // Distance calculated in JavaScript (Haversine formula) since we need all matching rows
          const rawCaregivers: any[] = await prisma.$queryRaw`
            ...
            WHERE u."approvalStatus" = 'APPROVED'
              AND u."isActive" = true
              AND (c."isAvailable" = ${isAvailable} OR u.id = 'cmega9vvm0004wmb4letzukpe')
              AND p.latitude IS NOT NULL
              AND p.longitude IS NOT NULL
              AND (${minRate}::float IS NULL OR c."hourlyRate" >= ${minRate}::float)
              AND (${maxRate}::float IS NULL OR c."hourlyRate" <= ${maxRate}::float)
              AND (${minRating}::float IS NULL OR c."averageRating" >= ${minRating}::float)
          ...
          const transformedCaregivers = caregivers.map((caregiver: any) => ({
            ...
            languages: caregiver.languages ? (
              typeof caregiver.languages === 'string' ? 
                (caregiver.languages.startsWith('[') ? JSON.parse(caregiver.languages) : caregiver.languages.split(',').map((lang: string) => lang.trim())) :
                caregiver.languages
            ) : [],

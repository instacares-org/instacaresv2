...
export class SMSService {
  ...
  async sendWelcomeSMS(
    ...
    const roleMessage = user.userType === 'CAREGIVER'
  ...
  async sendNewBookingToCaregiver(
    ...
    const body = `InstaCares: New booking from ${booking.parentName}! 📅 ${booking.date} at ${booking.time} (${booking.duration}hrs) for ${booking.childrenCount} child${booking.childrenCount > 1 ? 'ren' : ''}. Earnings: $${booking.totalAmount.toFixed(2)}. Check your dashboard!`;
   ...
   */
  async sendBookingConfirmationToParent(
    to: string,
    booking: {
      caregiverName: string;
      date: string;
    ...
    const body = `InstaCares: Your booking with ${booking.caregiverName} is confirmed! 📅 ${booking.date} at ${booking.time}. Total: $${booking.totalAmount.toFixed(2)}. Check your dashboard for details.`;

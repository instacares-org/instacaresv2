...
import { PaymentAuditLogger, AuditSeverity, AuditEventType } from '@/lib/audit/payment-audit';
...
export class PaymentAuthorizationService {
    ...
    try {
      const booking = await db.booking.findUnique({
        ...
        include: {
          ...
          caregiverUser: { select: { id: true, email: true } },
          caregiverProfile: { select: { userId: true } },
        },
      });
      ...
      const isCaregiver = booking.caregiverProfile?.userId === userId || booking.caregiverId === userId;
      ...
      if (!isOwner) {
        await this.auditLogger.logSecurityEvent({
          ...
          details: {
            ...
            caregiverId: booking.caregiverProfile?.userId,
            requiredRole
          },
  ...
  async verifyPaymentOwnership(
    ...
    try {
      ...
      const isOwner =
        ...
        payment.booking?.caregiverProfile?.userId === userId;
      ...
      if (!isOwner) {
        await this.auditLogger.logSecurityEvent({
          ...
          details: {
            ...
            bookingCaregiverId: payment.booking?.caregiverProfile?.userId,
          },
          severity: AuditSeverity.HIGH,
        });
      }
  ...
  async authorizePaymentOperation(
    ...
    await this.auditLogger.logPaymentEvent({
      type: AuditEventType.AUTHORIZATION_SUCCESS,
      userId: user.id,
  ...
  async checkRateLimit(
    ...
    try {
      ...
      const rateLimitAction = `RATE_LIMIT_${operation}`;
      ...
      await this.auditLogger.logPaymentEvent({
        ...
        userId,
        details: { operation, attempt: recentAttempts + 1, action: rateLimitAction },
      });

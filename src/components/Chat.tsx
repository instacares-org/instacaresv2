...
const Chat: React.FC<ChatProps> = ({ userId, userType }) => {
  ...
  const formatTime = (date: Date) => {
    ...
    return localTime.toFormat('h:mm a'); // e.g., "9:30 AM"
  };

  // Format date in user's timezone
  const formatDate = (date: Date) => {
    const dt = DateTime.fromJSDate(new Date(date), { zone: 'UTC' });
    const localDate = dt.setZone(userTimezone);
    const today = DateTime.now().setZone(userTimezone).startOf('day');
    const messageDate = localDate.startOf('day');
    ...
    return localDate.toFormat('MMM d'); // e.g., "Nov 9"
  };

...
export default function SimpleScheduleBuilder({ 
  ...
  useEffect(() => {
    const loadExistingSlots = async () => {
      try {
        ...
        console.log("🔍 SimpleScheduleBuilder - Loading slots with params:", { caregiverId, startDate, endDate });
        ...
        console.log("🔍 SimpleScheduleBuilder - Received slots:", slots?.length || 0, "slots:", slots);
      } catch (error) {
        ...
        if (duplicateSlot) {
          ...
          if (confirmReplace) {
            ...
            try {
              ...
              alert(`✗ Failed to replace existing slot: ${(deleteError as Error).message}`);
  ...
  const getExistingSlotsForDay = (date: Date) => {
    ...
    return existingSlots.filter(slot => {
      // FIX: Extract YYYY-MM-DD from ISO string directly (no timezone conversion)
      // The date field stores calendar dates, not timestamps
      const slotDateStr = slot.date.split('T')[0]; // "2025-11-16"

...
export default function ChildProfile({ child, onSave, onCancel, isOpen }: ChildProfileProps) {
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // If editing existing child with ID, upload to server immediately
      if (child?.id) {
        const photoFormData = new FormData();
        photoFormData.append("photo", file);
        photoFormData.append("childId", child.id);

        const response = await fetch("/api/children/upload-photo", {
          method: "POST",
        ...
        handleInputChange("photoUrl", data.photoUrl);
      } else {
        // For new children, store file temporarily and upload after child is created
        const photoUrl = URL.createObjectURL(file);
        handleInputChange("photoUrl", photoUrl);
        // Store the actual file for later upload
        (window as any).pendingChildPhoto = file;
      }
    } catch (error) {

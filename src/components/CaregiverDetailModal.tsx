...
export default function CaregiverDetailModal({ caregiver, isOpen, onClose, showContactInfo = false }: CaregiverDetailModalProps) {
  ...
  // Format time to user-friendly format in user's timezone
  const formatTime = (timeString: string) => {
    try {
      // DEBUG: Log the incoming timeString
      console.log('🕐 formatTime received:', timeString);
      
      // Ensure the time string is treated as UTC
      let utcTimeString = timeString;
      
      // If the string doesn't end with 'Z' and doesn't have a timezone offset, add 'Z'
      if (!timeString.endsWith('Z') && !timeString.includes('+') && !timeString.includes('T')) {
        // This might be a time-only string, skip it
        console.warn('Time-only string detected, returning as-is:', timeString);
        return timeString;
      } else if (!timeString.endsWith('Z') && !timeString.includes('+')) {
        // Has 'T' but no timezone - add 'Z' to force UTC interpretation
        utcTimeString = timeString.replace(/\.\d{3}$/, '') + 'Z';
        console.log('🔧 Normalized to UTC:', utcTimeString);
      }
      ...
      if (!dt.isValid) {
        console.warn('Invalid time format:', timeString, 'normalized:', utcTimeString, 'error:', dt.invalidReason);
        return timeString;
      }

      // Convert to user's timezone and format
      const localTime = dt.setZone(userTimezone);
      const formatted = localTime.toFormat('h:mm a ZZZZ'); // e.g., "9:00 AM EST"
      console.log('✅ Formatted:', timeString, '→', formatted);
      return formatted;
    } catch (error) {
  ...
  const formatAvailabilitySlots = () => {
    ...
    const groupedSlots = availabilitySlots.reduce((groups, slot) => {
      // FIX: Parse date as local date string, not as UTC timestamp
      // The date field stores calendar dates, not timestamps
      // Extract YYYY-MM-DD from the ISO string and parse as local date
      const dateString = slot.date.split('T')[0]; // "2025-11-16"
      const localDate = DateTime.fromISO(dateString, { zone: userTimezone });
      const date = localDate.toFormat('ccc, MMM d'); // e.g., "Sun, Nov 16"
                ...
                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-4 text-left align-middle shadow-xl transition-all pointer-events-auto">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-grow">
                          ...
                          {showContactInfo && caregiver.phone && (
                            <div className="flex items-center mt-1 text-gray-600 dark:text-gray-400">
                      ...
                      {/* Age Groups I Care For */}
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white mb-2 text-sm">Age Groups I Care For</h3>
                        {caregiver.ageGroups && caregiver.ageGroups.length > 0 ? (
                          <div className="grid grid-cols-2 gap-2">
                            {caregiver.ageGroups.map((group: any) => (
                              <div
                                key={group.id || group.name}
                                className="flex items-center space-x-2 px-3 py-2 bg-purple-50 dark:bg-purple-900/30 rounded-lg border border-purple-200 dark:border-purple-700"
                              >
                                <div className="flex-shrink-0 w-5 h-5 bg-purple-600 rounded flex items-center justify-center">
                                  <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                  </svg>
                                </div>
                                <div>
                                  <span className="text-sm font-medium text-purple-800 dark:text-purple-200">{group.name}</span>
                                  {group.description && (
                                    <span className="text-xs text-purple-600 dark:text-purple-400 ml-1">({group.description})</span>
                                  )}
                                </div>
                              </div>
                            ))}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex items-center space-x-2 px-3 py-2 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-200 dark:border-gray-600">
                              <div className="flex-shrink-0 w-5 h-5 bg-gray-400 rounded flex items-center justify-center">
                                <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                </svg>
                              </div>
                              <span className="text-sm text-gray-600 dark:text-gray-400">All Ages</span>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      {/* Services I Offer */}
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white mb-2 text-sm">Services I Offer</h3>
                        {caregiver.services && caregiver.services.length > 0 ? (
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400 italic">No services specified</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Daycare Photos */}
                      {/* Debug log removed - daycarePhotos rendering */}
                      <div>

...
const ChildrenCounter: React.FC<ChildrenCounterProps> = ({
  ...
  const formatChildrenCount = () => {
    if (totalChildren === 0) return t('search.addChildren');
    if (totalChildren === 1) return `1 ${t('search.child')}`;
    return `${totalChildren} ${t('search.children')}`;
  };
          ...
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-white rounded-2xl shadow-xl p-6 z-40 w-80 border border-gray-200">
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-gray-900">{t('search.infants')}</p>
                  <p className="text-sm text-gray-500">{t('search.infantsAge')}</p>
                </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-gray-900">{t('search.childrenLabel')}</p>
                  <p className="text-sm text-gray-500">{t('search.childrenAge')}</p>
                </div>
                <div className="flex items-center space-x-3">
                  <button 
              ...
              {totalChildren > 0 && (
                <div className="pt-4 border-t border-gray-100">
                  <p className="text-sm text-gray-600 text-center">
                    {t('search.total')}: <span className="font-medium text-gray-900">{formatChildrenCount()}</span>
                  </p>
                </div>
              )}
            </div>

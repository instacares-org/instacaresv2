...
import { csrfFetch } from './security/CSRFTokenProvider';
...
interface BookingFormProps {
  ...
  bookingDetails: {
    ...
    startTime: string;
    endTime: string;
    childrenCount?: number; // Optional for backwards compatibility
    selectedChildIds?: string[]; // Array of child IDs
...
export default function BookingForm({
  ...
  caregiver,
  totalAmount,
  commissionRate = 0.10, // Default to 10% if not provided
  onSuccess,
  onBack,
}: BookingFormProps) {
  ...
  // Calculate children count from either childrenCount or selectedChildIds array
  const getChildrenCount = () => {
    if (bookingDetails.childrenCount) {
      return bookingDetails.childrenCount;
    }
    if (bookingDetails.selectedChildIds && bookingDetails.selectedChildIds.length > 0) {
      return bookingDetails.selectedChildIds.length;
    }
    return 1; // Default to 1 if neither is available
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (isDemoMode) {
        ...
        // Create demo booking directly without calling Stripe API
        console.log('📤 Sending demo booking request:', {
          caregiverId: caregiver.id,
          date: bookingDetails.date,
          startTime: bookingDetails.startTime,
          endTime: bookingDetails.endTime,
          childrenCount: childrenCount,
          selectedChildIds: bookingDetails.selectedChildIds,
          hasLocation: !!caregiver.location
        });

        const demoBookingResponse = await csrfFetch('/api/bookings/demo', {
          method: 'POST',
          body: JSON.stringify({
            caregiverId: caregiver.id,
            date: bookingDetails.date,
            ...
            address: caregiver.location?.address || 'Demo address',
        ...
        const demoData = await demoBookingResponse.json();

        if (!demoBookingResponse.ok) {
          throw new Error(demoData.error || 'Failed to create booking');
        }

        console.log('✅ Demo booking created successfully:', demoData.bookingId);
        onSuccess();
        return;
      }

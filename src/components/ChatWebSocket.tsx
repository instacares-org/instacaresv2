      ...
      if (currentCount > previousCount && settings.messages && roomId !== selectedRoom) {
        ...
        newMessages.forEach(message => {
          ...
          if (!isFromCurrentUser) {
            ...
            showMessageRef.current(senderName, message.content, senderAvatar ?? undefined, roomId);
          }
        });
      }
            ...
            {rooms.length === 0 ? (
              ...
              rooms.map((room) => {
                ...
                const otherUserName = room.otherUser?.name ||
                  `${(otherUser as any)?.profile?.firstName || ''} ${(otherUser as any)?.profile?.lastName || ''}`.trim() ||
                  'Unknown User';
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages[selectedRoom]?.length === 0 ? (
                  ...
                  messages[selectedRoom]?.map((message) => {
                    ...
                    const senderAvatar = message.senderAvatar || (message.sender as any)?.profile?.avatar;

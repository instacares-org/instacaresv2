...
// Fetch platform settings  useEffect(() => {    const fetchPlatformSettings = async () => {      try {        const response = await fetch('/api/admin/settings');        if (response.ok) {          const data = await response.json();          setShowContactInfo(data.settings?.showCaregiverContactInfo ?? false);        }      } catch (error) {        // Silently fail - default to not showing contact info        console.log('Could not fetch platform settings');      }    };    fetchPlatformSettings();  }, []);
  ...
  const fetchBabysitters = async () => {
    try {
      const response = await fetch('/api/babysitters?limit=12&sortBy=rating', {
  ...
  // Merge caregivers and babysitters based on provider type filter
  const filteredBabysitters = useMemo(() => {
    if (activeFilters.providerType === 'caregivers') return [];
    let filtered = [...allBabysitters];

    // Price filter
    if (activeFilters.priceRange !== 'any') {
      filtered = filtered.filter(b => {
        switch (activeFilters.priceRange) {
          case '15-25': return b.hourlyRate >= 15 && b.hourlyRate <= 25;
          case '25-35': return b.hourlyRate >= 25 && b.hourlyRate <= 35;
          case '35-45': return b.hourlyRate >= 35 && b.hourlyRate <= 45;
          case '45+': return b.hourlyRate >= 45;
          default: return true;
        }
      });
    }
    ...
    if (activeFilters.experience !== 'any') {
      filtered = filtered.filter(b => {
        switch (activeFilters.experience) {
          case 'new': return b.experienceYears >= 0 && b.experienceYears <= 2;
          case 'experienced': return b.experienceYears >= 3 && b.experienceYears <= 7;
          case 'expert': return b.experienceYears >= 8;
          default: return true;
        }
      });
    }
    ...
    return filtered;
  }, [allBabysitters, activeFilters]);

  const showCaregivers = activeFilters.providerType !== 'babysitters';
  const displayedCaregivers = showCaregivers ? caregivers : [];
  const hasResults = displayedCaregivers.length > 0 || filteredBabysitters.length > 0;
      ...
      <main>
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 mb-2">
            <h2 className="text-black dark:text-white text-2xl font-semibold">{t('home.verifiedProviders')}</h2>
            {userLocation && (
              <div className="flex items-center gap-4">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="inline-flex items-center gap-2">
                    ...
                    {t('home.showingProviders', { radius: selectedRadius })}
                  </span>
                </div>
                <div className="flex items-center space-x-3">
                  <label className="text-xs font-semibold text-gray-700 dark:text-gray-300 tracking-wide">{t('home.distance')}:</label>
                  <div className="relative">
                    <select 
                      value={selectedRadius} 
          ...
          {!loading && !error && !hasResults && (
            <div className="text-center py-12">
              <p className="text-gray-600 dark:text-gray-400">No providers found in your area.</p>
            </div>
          )}

...
export async function POST(request: NextRequest) {
  try {
    ...
    type BookingWithRelations = typeof allBookings[number];
    const bookingGroups = new Map<string, BookingWithRelations[]>();
      ...
      .map(([key, group]) => {
        ...
        return {
          ...
          bookings: group.map((b, index) => ({
            ...
            caregiverName: `${b.caregiverUser.profile?.firstName} ${b.caregiverUser.profile?.lastName}`,

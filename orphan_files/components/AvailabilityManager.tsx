...
export default function AvailabilityManager({ 
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      ...
      setSlots((availableSlots || []) as unknown as TimeSlot[]);
    } catch (err) {
...
function TimeSlotForm({
  ...
  const { timezone: formTimezone } = useUserTimezone();

  // Convert UTC dates/times to user's local timezone for form pre-population
  const getLocalDate = (utcDateStr: string) => {
    const dt = DateTime.fromISO(utcDateStr.endsWith('Z') ? utcDateStr : utcDateStr + 'Z', { zone: 'UTC' }).setZone(formTimezone);
    return dt.isValid ? dt.toFormat('yyyy-MM-dd') : DateTime.now().setZone(formTimezone).toFormat('yyyy-MM-dd');
  };
  const getLocalTime = (utcTimeStr: string) => {
    const dt = DateTime.fromISO(utcTimeStr.endsWith('Z') ? utcTimeStr : utcTimeStr + 'Z', { zone: 'UTC' }).setZone(formTimezone);
    return dt.isValid ? dt.toFormat('HH:mm') : '09:00';
  };

  const [formData, setFormData] = useState({
    date: slot?.date ? getLocalDate(slot.date) : DateTime.now().setZone(formTimezone).toFormat('yyyy-MM-dd'),
    startTime: slot?.startTime ? getLocalTime(slot.startTime) : '09:00',
    endTime: slot?.endTime ? getLocalTime(slot.endTime) : '17:00',

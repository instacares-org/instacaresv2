...
async function main() {
  ...
  // Create test accounts for user testing
  const testParentPasswordHash = await bcrypt.hash('Test123!@#', 10);
  const testParent = await prisma.user.upsert({
    where: { email: 'test.parent@instacares.com' },
    update: {},
    create: {
      email: 'test.parent@instacares.com',
      passwordHash: testParentPasswordHash,
      userType: 'PARENT',
      approvalStatus: 'APPROVED',
      emailVerified: new Date(),
      profile: {
  ...
  const testCaregiverPasswordHash = await bcrypt.hash('Test123!@#', 10);
  const testCaregiverUser = await prisma.user.upsert({
    where: { email: 'test.caregiver@instacares.com' },
    update: {},
    create: {
      email: 'test.caregiver@instacares.com',
      passwordHash: testCaregiverPasswordHash,
      userType: 'CAREGIVER',
      approvalStatus: 'APPROVED',
      emailVerified: new Date(),
      profile: {
  ...
  const testCaregiver = await prisma.caregiver.upsert({
    where: { userId: testCaregiverUser.id },
    update: {},
    create: {
      userId: testCaregiverUser.id,
      hourlyRate: 25.00,
      experienceYears: 3,
      bio: 'Test caregiver account for development and testing purposes.',
      languages: ['English'],
      maxChildren: 3,
      minAge: 0,
      maxAge: 144,
  ...
  console.log('\n🧪 Test Accounts:');
  console.log(`👨‍👩‍👧‍👦 Test Parent: test.parent@instacares.com / Test123!@#`);
  console.log(`👶 Test Caregiver: test.caregiver@instacares.com / Test123!@#`);
}
